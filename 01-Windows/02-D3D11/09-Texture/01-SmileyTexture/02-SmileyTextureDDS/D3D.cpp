...
HRESULT initialize(void)
{
	...
	void uninitialize(void);
	HRESULT resize(int, int);
	HRESULT LoadD3DTextureDDS(TCHAR[], ID3D11ShaderResourceView**);
	...
	// Create Texture
	hr = LoadD3DTextureDDS(MAKEINTRESOURCE(SMILEY_DDS_TEXTURE), &gpID3D11ShaderResourceView_Texture_Smiley);
	if(FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "initialize:LoadD3DTextureDDS() failed for Smiley.bmp.\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "initialize:LoadD3DTextureDDS() Successful for Smiley.bmp.\n");
		fclose(gpFile);
	}
...
HRESULT LoadD3DTextureDDS(TCHAR imageResourceID[], ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	// Variable Declarations
	HRESULT hr = S_OK;
	// Code
	// Find Resource
	HRSRC hrc = FindResource(GetModuleHandle(NULL), imageResourceID, RT_RCDATA);
	// HRSRC hrc = FindResourceEx(GetModuleHandle(NULL), MAKEINTRESOURCE(SMILEY_DDS_TEXTURE), RT_RCDATA, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL));
	if(hrc == NULL)
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:FindResource failed.\n");
		fclose(gpFile);
		return E_FAIL;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:FindResource Successful.\n");
		fclose(gpFile);
	}

	HGLOBAL hGlobal = LoadResource(GetModuleHandle(NULL), hrc);
	if(hGlobal == NULL)
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:LoadResource failed.\n");
		fclose(gpFile);
		return E_FAIL;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:LoadResource Successful.\n");
		fclose(gpFile);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:LockResource failed.\n");
		fclose(gpFile);
		return E_FAIL;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:LockResource Successful.\n");
		fclose(gpFile);
	}

	DWORD resourceSize = SizeofResource(GetModuleHandle(NULL), hrc);
	if(resourceSize == 0)
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:SizeofResource failed.\n");
		fclose(gpFile);
		return E_FAIL;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:SizeofResource Successful.\n");
		fclose(gpFile);
	}

	hr = DirectX::CreateDDSTextureFromMemory(gpID3D11Device, (const uint8_t*)resourceData, (size_t)resourceSize, NULL, ppID3D11ShaderResourceView);
	if(FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:CreateDDSTextureFromMemory failed.\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "+a");
		fprintf(gpFile, "LoadD3DTextureDDS:CreateDDSTextureFromMemory Successful.\n");
		fclose(gpFile);
	}
	return hr;
}
