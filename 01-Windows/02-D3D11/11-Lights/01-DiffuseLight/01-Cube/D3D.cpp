...
HRESULT initialize(void)
{
	...
	// C.Create actual buffer
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_NormalBuffer_Cube);
	if(FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
...
void display(void)
{
	...
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_NormalBuffer_Cube, &stride, &offset);
