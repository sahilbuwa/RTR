...
HRESULT initialize(void)
{
	...
	"float3 diffuse_light_color:COLOR;" \
	"};" \
	...
	"output.diffuse_light_color = ld * kd * max(dot(lightDirection, transformedNormals), 0.0);" \
	"}" \
	"else" \
	"{" \
	"output.diffuse_light_color = float3(1.0, 1.0, 1.0);" \
	"}" \
	...
	"{" \
	"return float4(input.diffuse_light_color, 1.0);"
	"}";
