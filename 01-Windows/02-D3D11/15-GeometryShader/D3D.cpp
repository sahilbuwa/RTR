...
HRESULT initialize(void)
{
	...
	const char* vertexShaderSourceCode =
		...
		"{" \
		"float4x4 worldViewProjectionMatix;" \
		"}" \
		...
		"output.position = mul(worldViewProjectionMatix, pos);" \
		"return output;" \
		"}";
	...
	const char* geometryShaderSourceCode =
		...
		"for(int i = 0; i < 3; i++)" \
		"{" \
		"output.position = mul(worldViewProjectionMatix, (input[i].position + float4(0.0, 1.0, 0.0, 1.0)));" \
		"triangleStream.Append(output);" \
		"output.position = mul(worldViewProjectionMatix, (input[i].position + float4(1.0, -1.0, 0.0, 1.0)));" \
		"triangleStream.Append(output);" \
		"output.position = mul(worldViewProjectionMatix, (input[i].position + float4(-1.0, -1.0, 0.0, 1.0)));" \
		"triangleStream.Append(output);" \
	...
	hr = D3DCompile(geometryShaderSourceCode, lstrlenA(geometryShaderSourceCode) + 1,"GS",NULL,D3D_COMPILE_STANDARD_FILE_INCLUDE,"main","gs_5_0",0,0,&pID3DBlob_GeometryShaderCode,&pID3DBlob_Error);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "D3DCompile() Succeded For Geometry Shader.\n");
		fclose(gpFile);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "CreateGeometryShader() Failed.\n");
		fclose(gpFile);
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf(gpFile, "CreateGeometryShader() Succeded.\n");
		fclose(gpFile);
	}

	// Set this geometry shader in geometry shader stage in geometry shader stage of pipeline
	...
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor, &d3d11SubresourceData, &gpID3D11Buffer_Position_Triangle_Buffer);
	if(FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
...
void display(void)
{
	...
	UINT offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_Position_Triangle_Buffer, &stride, &offset);
