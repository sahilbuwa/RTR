...
public class GLESView extends GLSurfaceView implements OnDoubleTapListener, OnGestureListener, GLSurfaceView.Renderer
{
	...
	private byte checkerboard[] = new byte[Checkerboard_Width * Checkerboard_Height * 4];
	...
	private void initialize()
	{
		...
		ByteBuffer byteBuffer = ByteBuffer.allocateDirect(texcoords.length * 4); // sizeof replacement
		byteBuffer.order(ByteOrder.nativeOrder());
		FloatBuffer texcoordsBuffer = byteBuffer.asFloatBuffer();
		texcoordsBuffer.put(texcoords);
		texcoordsBuffer.position(0);
		GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, texcoords.length * 4, texcoordsBuffer, GLES32.GL_STATIC_DRAW);
		...
		// Clearing the screen to gray color
	...
	private int loadGLTexture()
	{
		...
		bitmap.copyPixelsFromBuffer(ByteBuffer.wrap(checkerboard));
		...
		// // Create The Texture
		// GLES32.glTexImage2D(GLES32.GL_TEXTURE_2D, 0, GLES32.GL_RGBA, Checkerboard_Width, Checkerboard_Height, 0, GLES32.GL_RGBA, GLES32.GL_UNSIGNED_BYTE, ByteBuffer.wrap(checkerboard));
	...
	private void makeCheckerBoard()
	{
		...
		for(int i = 0; i < Checkerboard_Height; i++)
		{
			for(int j = 0; j < Checkerboard_Width; j++)
			{
				...
				checkerboard[(i * Checkerboard_Width + j) * 4 + 0] = (byte)c;
				checkerboard[(i * Checkerboard_Width + j) * 4 + 1] = (byte)c;
				checkerboard[(i * Checkerboard_Width + j) * 4 + 2] = (byte)c;
				checkerboard[(i * Checkerboard_Width + j) * 4 + 3] = (byte)0xff;
			}
		}
	}
	...
	private void display()
	{
		...
		byteBuffer = ByteBuffer.allocateDirect(position.length * 4); // sizeof replacement
		byteBuffer.order(ByteOrder.nativeOrder());
