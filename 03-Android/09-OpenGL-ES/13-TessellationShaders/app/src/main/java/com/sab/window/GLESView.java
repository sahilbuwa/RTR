...
public class GLESView extends GLSurfaceView implements OnDoubleTapListener, OnGestureListener, GLSurfaceView.Renderer
{
	...
	private void initialize()
	{
		...
		if(status[0] == GLES32.GL_FALSE)
		{
			...
			if(infoLogLength[0] > 0)
			{
				log = GLES32.glGetShaderInfoLog(tessellationControlShaderObject);
				System.out.println("SAB: Tessellation Control Shader Compilation log : "+log);
				uninitialize();
				System.exit(0);
			}
		}
		...
		final String tessellationEvaluationShaderSourceCode = String.format(
			...
			"vec3 p = p0 * (1.0f -u) * (1.0f-u) * (1.0f-u) + p1 * 3.0f * u * (1.0f-u) * (1.0f-u) + p2 * 3.0f * u * u * (1.0f-u) + p3 * u * u * u;\n" +
		...
		if(status[0] == GLES32.GL_FALSE)
		{
			...
			if(infoLogLength[0] > 0)
			{
				log = GLES32.glGetShaderInfoLog(tessellationEvaluationShaderObject);
				System.out.println("SAB: Tessellation Evaluation Shader Compilation log : "+ log);
				uninitialize();
				System.exit(0);
			}
		}
		...
		GLES32.glVertexAttribPointer(MyGLESMacros.SAB_ATTRIBUTE_POSITION, 2, GLES32.GL_FLOAT, false, 0, 0);
	...
	private void display()
	{
		...
		if(isDecreasing)
			GLES32.glUniform4f(lineColorUniform, 1.0f, 0.5f, 0.0f, 1.0f);
		else
			GLES32.glUniform4f(lineColorUniform, 1.0f, 1.0f, 0.0f, 1.0f);

		GLES32.glUniform1i(numberOfStripsUniform, 1); // 1 strip is there.
