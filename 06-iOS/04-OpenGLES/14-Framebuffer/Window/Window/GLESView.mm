...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if(self)
    {
		...
		{
			printf("Incomplete Framebuffer initwithframe.\n");
			[self uninitialize];
			[self release];
			exit(0);
		}
...
- (void)layoutSubviews
{
	...
	if(glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
	{
		printf("Incomplete Framebuffer layoutsubviews.\n");
		[self uninitialize];
		[self release];
		exit(0);
	}
...
-(bool)createFBO:(GLint)textureWidth :(GLint)textureHeight
{
	...
	{
		printf("Insufficient renderbuffer size.\n");
		return NO;
	}
	...
	if(status == GL_FALSE)
	{
		...
		if(infoLogLength > 0)
		{
			...
			if(log != NULL)
			{
				...
				printf("Vertex Shader of sphere Compilation Log : %s\n", log);
				free(log);
	...
	if(status == GL_FALSE)
	{
		...
		if(infoLogLength > 0)
		{
			...
			if(log != NULL)
			{
				...
				printf("Fragment Shader of sphere Compilation Log : %s\n", log);
				free(log);
				[self uninitialize_sphere];
			}
		}
