...
int initialize(void)
{
	...
	// Select CUDA suppoted Device
	cudaSetDevice(0);	// Selecting the default 1st CUDA supported device
	...
	// Create CUDA OpenGL interoperabiliity resource
...
void display(void)
{
	...
	if(onGPU == TRUE)
	{
		...
		// Ask CUDA to give mapped pointer of mapped result
		...
		sineWaveKernel<<<grid, block>>>(pPos, meshWidth, meshHeight, animationTime);
		// Unmap the given mapped pointer to use in cpu
		...
		glBindBuffer(GL_ARRAY_BUFFER, vbo_gpu); // As CUDA does all the work no need of following 3 lines
	}
	else
	{
		// CPU Related Code
